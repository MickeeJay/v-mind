...
Clarinet.test({
  name: 'security: strategy-vault and receipt-token asset accounting stay synchronized',
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const setup = chain.mineBlock([
      ...
      Tx.contractCall('strategy-vault', 'create-vault', [types.principal(asset.address), types.uint(4_000_000), types.uint(1)], deployer.address),
      Tx.contractCall('strategy-vault', 'deposit', [types.uint(1), types.principal(asset.address), types.uint(1_000_000)], deployer.address),
      Tx.contractCall('strategy-vault', 'withdraw', [types.uint(1), types.uint(1_500_000)], deployer.address),
    ]);
    ...
    const vaultAssets = chain.callReadOnlyFn('strategy-vault', 'get-vault-total-assets', [types.uint(1)], deployer.address);
