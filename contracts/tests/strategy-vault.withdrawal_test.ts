...
Clarinet.test({
  name: 'vault-core: supports partial and full withdrawal for owner',
...
Clarinet.test({
  name: 'vault-core: withdrawal rejects insufficient balance and non-owner callers',
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get('deployer')!;
    ...
    const block = chain.mineBlock([
      ...
      Tx.contractCall('vault-core', 'withdraw', [types.uint(1), types.uint(3_000_000)], deployer.address),
      Tx.contractCall('vault-core', 'withdraw', [types.uint(1), types.uint(500_000)], nonOwner.address),
    ]);
...
Clarinet.test({
  name: 'vault-core: withdrawal blocked while strategy execution lock is active',
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get('deployer')!;
    const protocol = accounts.get('wallet_1')!;
    const executor = accounts.get('wallet_2')!;
    ...
    const block = chain.mineBlock([
      ...
      Tx.contractCall('vault-core', 'withdraw', [types.uint(1), types.uint(500_000)], deployer.address),
    ]);
