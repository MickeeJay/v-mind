...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const setup = chain.mineBlock([
      ...
      Tx.contractCall('vault-core', 'set-max-aum-drop-bps-per-tx', [types.uint(500)], deployer.address),
    ]);
    ...
    const excessiveFee = chain.mineBlock([
      Tx.contractCall('vault-core', 'apply-performance-fee', [types.uint(1), types.uint(1_000_000)], deployer.address),
    ]);

    excessiveFee.receipts[0].result.expectErr().expectUint(2419);

    const safeFee = chain.mineBlock([
      Tx.contractCall('vault-core', 'apply-performance-fee', [types.uint(1), types.uint(400_000)], deployer.address),
    ]);
