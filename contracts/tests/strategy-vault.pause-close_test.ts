...
Clarinet.test({
  name: 'strategy-vault: owner can pause and unpause vault, non-owner cannot pause',
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const block = chain.mineBlock([
      ...
      Tx.contractCall('strategy-vault', 'pause-vault', [types.uint(1)], nonOwner.address),
      Tx.contractCall('strategy-vault', 'unpause-vault', [types.uint(1)], deployer.address),
    ]);
...
Clarinet.test({
  name: 'strategy-vault: close requires zero balance and marks vault closed',
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get('deployer')!;
    const protocol = accounts.get('wallet_1')!;
    const executor = accounts.get('wallet_2')!;
