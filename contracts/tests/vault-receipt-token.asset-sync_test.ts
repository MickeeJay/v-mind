...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const block = chain.mineBlock([
      ...
      Tx.contractCall('vault-core', 'accrue-yield', [types.uint(1), types.uint(300_000)], deployer.address),
      Tx.contractCall('vault-core', 'apply-performance-fee', [types.uint(1), types.uint(100_000)], deployer.address),
