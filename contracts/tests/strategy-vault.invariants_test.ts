...
Clarinet.test({
  name: 'strategy-vault invariant: balance never becomes negative on over-withdraw',
...
Clarinet.test({
  name: 'strategy-vault invariant: only owner can withdraw from vault',
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get('deployer')!;
...
Clarinet.test({
  name: 'strategy-vault invariant: closed vault cannot receive deposits',
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get('deployer')!;
    const protocol = accounts.get('wallet_1')!;
    const executor = accounts.get('wallet_2')!;
    ...
    const block = chain.mineBlock([
      ...
      Tx.contractCall('strategy-vault', 'close-vault', [types.uint(1)], deployer.address),
...
Clarinet.test({
  name: 'strategy-vault invariant: cumulative deposits cannot exceed configured asset max',
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get('deployer')!;
    const protocol = accounts.get('wallet_1')!;
    const executor = accounts.get('wallet_2')!;
    ...
    const block = chain.mineBlock([
      ...
      Tx.contractCall('strategy-registry', 'register-strategy', [types.ascii('Yield'), types.uint(1), types.principal(protocol.address), types.uint(1), types.principal(executor.address)], deployer.address),
      Tx.contractCall('strategy-vault', 'create-vault', [types.principal(protocol.address), types.uint(2_000_000), types.uint(1)], deployer.address),
      Tx.contractCall('strategy-vault', 'deposit', [types.uint(1), types.principal(protocol.address), types.uint(2_000_000)], deployer.address),
    ]);
